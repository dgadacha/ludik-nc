{**
 * Fiche produit Ludik : étiquette du rayon au-dessus du titre, prix aligné
 * à droite, bloc de réassurance sous l'ajout au panier.
 *}
{extends file=$layout}
        ...
        <div class="product__heading">
          <div class="product__heading-main">
            {if !empty($product.category_name)}
              <span class="lud-tag">{$product.category_name}</span>
            {/if}
            <h1 class="product__name h2">
              {block name='page_title'}{$product.name}{/block}
            </h1>
            {if !empty($product.ean13)}
              <p class="product__ean">EAN13 : {$product.ean13}</p>
            {/if}
          </div>
      ...
      <div class="product__actions js-product-actions">
        {block name='product_buy'}
          <form action="{$urls.pages.cart}" method="post" id="add-to-cart-or-refresh">
            ...
            {include file='_partials/ludik-trust-product.tpl'}
